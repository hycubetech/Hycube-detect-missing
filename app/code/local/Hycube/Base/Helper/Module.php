 ...
 * @package Hycube_Base
 */
class Hycube_Base_Helper_Module extends Mage_Core_Helper_Abstract
{
    ...
    const URL_EXTENSIONS  = 'http://hycube.com/feed-extensions.xml';
    ...
    protected function _isHycubeModule()
    {
        return strpos($this->_controllerModule, "Hycube") !== FALSE && $this->_getInstalledVersion() !== NULL;
    }

    static function baseModuleInstalled()
    {
        $ret = Mage::getStoreConfig(Hycube_Base_Helper_Module::INSTALLED_PATH);
        if (!$ret)
        {
            $ret = time();
            Mage::getConfig()->saveConfig(Hycube_Base_Helper_Module::INSTALLED_PATH, $ret);
    ...
    {
        $path = Hycube_Base_Helper_Module::UPDATED_PREFIX . $this->_controllerModule;
    ...
    {
        $ret = false;
        if ($this->_isHycubeModule() && $this->_validateBaseModulePeriod() && $this->_validateModulePeriod())
        {
